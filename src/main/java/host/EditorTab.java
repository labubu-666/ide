...
class EditorTab extends Tab {
    ...
    EditorTab(String title, String initialText, String ext, Path path, EditorContext ctx) {
        ...
        codeArea.addEventFilter(KeyEvent.KEY_PRESSED, event -> {
            ...
            if (event.getCode() == KeyCode.TAB) {
                handleIndentation(event.isShiftDown());
                event.consume();
                return;
            }
    ...
    private void handleIndentation(boolean outdent) {
        ...
        int oldCaretPosition = codeArea.getCaretPosition();
        codeArea.replaceText(0, text.length(), newText.toString());

        int newStart = 0;
        for (int i = 0; i < startLine; i++) {
            String modifiedLine = newText.toString().split("\n", -1)[i];
            newStart += modifiedLine.length() + 1;
        }
